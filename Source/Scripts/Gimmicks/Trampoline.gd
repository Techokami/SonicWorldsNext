...
#spriteFrameCount - Set this to the number of frames in the sprite Texture
...
func _process(_delta):
	if Engine.is_editor_hint():
		$TrampolineBody/TrampolineShape.get_shape().set_extents(Vector2(platformWidth, platformHeight / 2))
