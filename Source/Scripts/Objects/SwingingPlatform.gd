...
export(float,1.0,180.0) var rotate_amount = 90.0 # How far do you want the swings to rotate?
...
func _ready():
	if(!Engine.editor_hint): # If not in editor, show the platform, and set the platform's image
		...
		platform.get_node("Shape").shape.extents.x = plat_img.get_size().x/2
...
func _draw():
	...
	draw_arc(Vector2.ZERO,chains*chain_size + float(chain_size)/2,deg2rad(90+rotate_amount),deg2rad(90-rotate_amount),chain_size,Color(0.5,0,1,0.5),2)
	...
	temppos = Vector2(sin(deg2rad(0)) * (chains * chain_size + (float(chain_size) / 2)),cos(deg2rad(0)) * (chains * chain_size + (float(chain_size) / 2)))
	temppos -= Vector2(float(plat_img.get_width()) / 2,float(plat_img.get_height()) / 2)
	draw_texture(plat_img,temppos,Color(1,1,1,0.25))
	# Draw the possible right position
	temppos = Vector2(sin(deg2rad(rotate_amount)) * (chains * chain_size + (float(chain_size) / 2)),cos(deg2rad(rotate_amount)) * (chains * chain_size + (float(chain_size) / 2)))
	temppos -= Vector2(float(plat_img.get_width()) / 2,float(plat_img.get_height()) / 2)
	draw_texture(plat_img,temppos,Color(1,1,1,0.25))
